...
we use the git commit messages to **generate the Library change log**.
...
- `alpha` version will be created per pull request. `npm i @wiooh/<package-name>@alpha`
- `beta` version will be created per PR are merged to `develop` branch. Once the PRs are merged to `develop`. `npm i @wiooh/<package-name>@beta`
- Stable version will be created per PR are merged to `master` branch. managed by WiOOH. `npm i @wiooh/<package-name>`